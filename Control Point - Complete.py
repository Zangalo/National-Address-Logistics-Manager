...
def gerar_grafico(dados):
    ...
    plt.title("Distribuição de Encomendas por Região")
    plt.axis("equal")
    plt.show()
...
# Programa principal
def main():
    dados_estado = agrupar_por_estado(CEPS)

    dados_regiao = agrar_por_regiao = agrupar_por_regiao(dados_estado)

    print("\nResumo por região:")
    for regiao, lista in dados_regiao.items():
        print(f"{regiao}: {len(lista)} endereços")

    salvar_por_regiao(dados_regiao)
    gerar_grafico(dados_regiao)
